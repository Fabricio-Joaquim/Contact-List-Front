...
const schema = yup.object({
	...
	phone: yup.string()
		...
		.required(),
	email: yup.string().email().required().max(numberMMax, 'No máximo ' + numberMMax + ' caracteres  no campo ${path}').max(numberMMax, 'No máximo ' + numberMMax + ' caracteres  no campo ${path}')
	,
	address: yup.string().required().max(numberMMax, 'No máximo ' + numberMMax + ' caracteres  no campo ${path}').max(numberMMax, 'No máximo ' + numberMMax + ' caracteres  no campo ${path}')
	,
	dateBorn: yup.string('campo de Data de Nascimento é').required(),
}).required();

const schemaUserRegister = yup.object({
	password: yup.string().min(6, 'Ao menos ' + 6 + ' caracteres  no campo ${path}').max(20, 'No máximo ' + 20 + ' caracteres  no campo ${path}').required(),
